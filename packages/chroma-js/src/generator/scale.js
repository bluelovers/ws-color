        ...
        if (_useCache && _colorCache[k]) {
            ...
            if (Array.isArray(_colors)) {
                ...
                for (let i = 0; i < _pos.length; i++) {
                    ...
                    if (t > p && t < _pos[i + 1]) {
                        t = (t - p) / (_pos[i + 1] - p);
                        col = chroma_1.default.interpolate(_colors[i], _colors[i + 1], t, _mode);
                        break;
                    }
                }
            }
    ...
    f.classes = function (classes) {
        if (classes != null) {
            if (Array.isArray(classes)) {
                ...
                const d = chroma_1.default.analyze(_domain);
                if (classes === 0) {
                    _classes = [d.min, d.max];
                }
                else {
                    _classes = chroma_1.default.limits(d, 'e', classes);
                }
            }
            return f;
        }
