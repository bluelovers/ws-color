...
): IScale<OutType>
{
	...
	let _colorCache: Record<string, Color> = {};
	let _useCache = true;
	let _gamma = 1;
	...
	const f = function (v: number): Color
	{
		const c = chroma(getColor(v));
		if (_out && c[_out as any as keyof Color])
		{ return c[_out as any](); }
		else
		{ return c; }
	};
	...
	f.colors = function (...argv: [number?, string?])
	{
